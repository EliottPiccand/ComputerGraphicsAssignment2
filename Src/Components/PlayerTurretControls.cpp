...
void PlayerTurretControls::update(float deltaTime)
{
    ...
    aimingValidPosition = Input::isMouseInWindow();
    const glm::vec2 cursorPosition = Singleton::camera.lock()->toWorldPosition(Input::getMousePos());

    // Keep aiming at the latest valid cursor position. If cursor leaves the game window,
    // keep the previous target so the turret stays stable.
    if (aimingValidPosition)
    {
        lastKnownAimTarget = std::optional(cursorPosition);
    }

    if (lastKnownAimTarget.has_value())
    {
        const glm::vec2 direction = glm::normalize(lastKnownAimTarget.value() - position);
    ...
    if (Input::getState(Input::Action::Fire) == Input::State::JustPressed && lastKnownAimTarget.has_value())
    {
        previewTarget = lastKnownAimTarget;
    }
    else if (previewTarget.has_value() && lastKnownAimTarget.has_value())
    {
        previewTarget = lastKnownAimTarget;
    }
    ...
    if (Input::getState(Input::Action::Fire) == Input::State::JustReleased && previewTarget.has_value())
    {
        if (aimingValidPosition)
        {
